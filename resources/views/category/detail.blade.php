						...
						<div class="book-list-widget masonry-grid business">
							...
							<div class="book-list-detail">
								@if($book->book_img == 'No image found')
									<img src="/assets/images/no-image.jpg" width="109" height="164" alt="No image found">
								@else
									<img src="/uploads/{{ $book->book_img }}" width="109" height="164" alt="{{ $book->title }}">
								@endif
								<div class="detail">
									<span>by {{ $book->author_book->title }}</span>
									<div class="book-name">
										<h5>{{ $book->title }}<span>{{ $book->category->title }}</span></h5><strong>PKR {{ number_format($book->price) }}</strong>
									</div>
