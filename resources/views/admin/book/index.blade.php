...
<div class="box">
      ...
      <div class="box-body">
        ...
        <table class="table table-bordered">
          ...
          <tr>
            <td><input type="checkbox" name="statusAll[]" value="{{ $book->id }}" class="checkSingle"></td>
            <td>{{$book->title}}</td>
            <td>{{$book->author_id}}</td>
            <td>{{$book->category_id}}</td>
            <td>
              @if ($book->book_img == 'No image found')
                <img src="/assets/admin/dist/img/no-image.png" width="100" height="100" class="img-thumbnail" alt="No image found">
              @else
    ...
    $(document).ready(function() {
       ...
       $(".singleStatus").on('click', function(event) {
           ...
           var url = self.closest('form').attr('action');
            ...
            self.html('<img src="/assets/admin/dist/img/spinner.gif">');
           $.ajax({
               url: url,
               type: 'PUT',
           })
